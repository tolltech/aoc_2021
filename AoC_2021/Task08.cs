    ...
    public class Task08
    {
        ...
        public void Solve(string input, int expected)
        {
            ...
            foreach (var line in lines)
            {
                ...
                var words = splits[1].Split(" ", StringSplitOptions.RemoveEmptyEntries);
                acc += words.Count(x => new[] { 2, 3, 4, 7 }.Contains(x.Length));
            }
        ...
        {
            yield return new TestCaseData(@"be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
