    ...
    public class Task18_2
    {
        ...
        public void Solve(string input, int expected)
        {
            ...
            for (var j = 0; j < lines.Length; j++)
            {
                ...
                var treeLeft = Parse(lines[i]);
                var treeRight = Parse(lines[j]);

                var tree = Add(treeLeft, treeRight);

                while (true)
                {
                    if (!Reduce(tree)) break;
                }

                var mg = Magnitude(tree);
                if (result < mg)
                {
                    result = mg;
                }
        ...
        private static bool Explode(Node tree)
        {
            ...
            var leftRegular = allNodes.TakeWhile(x => x.Node != mostLeft.Left)
                .LastOrDefault(x => x.Node.Value.HasValue);
...
[[[[1,1],2],1],[1,[[6,4],2]]]
[[[[6,3],[1,5]],[6,[7,7]]],[6,6]]
[[[[3,0],[5,6]],1],[[[9,3],[1,7]],[[3,4],[2,7]]]]", 4536);
        }
