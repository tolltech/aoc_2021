...
namespace AoC_2021
{
    ...
    {
        private static readonly Dictionary<int, int[]> numbersBySegments = new Dictionary<int, int[]>
        {
            { 2, new[] { 1 } },
            { 3, new[] { 7 } },
            { 4, new[] { 4 } },
        ...
        public void Solve(string input, int expected)
        {
            ...
            foreach (var line in lines)
            {
                ...
                var allNumbers = splits[0].Split(" ", StringSplitOptions.RemoveEmptyEntries);
                var numbers = splits[1].Split(" ", StringSplitOptions.RemoveEmptyEntries);

                var numberSets = new string[10];

                numberSets[1] = allNumbers.Single(x => x.Length == 2);
                numberSets[4] = allNumbers.Single(x => x.Length == 4);
                numberSets[7] = allNumbers.Single(x => x.Length == 3);
                numberSets[8] = allNumbers.Single(x => x.Length == 7);

                numberSets[2] = allNumbers.Single(x => x.Length == 5 && x.Intersect(numberSets[4]).Count() == 2);
                numberSets[5] = allNumbers.Single(x => x.Length == 5 && x.Intersect(numberSets[2]).Count() == 3);
                numberSets[3] = allNumbers.Single(x => x.Length == 5 && x.Intersect(numberSets[5]).Count() == 4);

                numberSets[9] = allNumbers.Single(x => x.Length == 6 && x.Intersect(numberSets[3]).Count() == 5);
                numberSets[6] = allNumbers.Single(x => x.Length == 6 && x.Intersect(numberSets[1]).Count() == 1);
                numberSets[0] = allNumbers.Except(numberSets).Single();
                ...
                foreach (var number in numbers)
                {
                    ...
                    {
                        if (numberSets[i].Length == number.Length &&
                            numberSets[i].Intersect(number).Count() == number.Length)
                        {
                            s += i.ToString();
                            break;
                        }
