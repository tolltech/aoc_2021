...
namespace AoC_2021
{
    ...
    public class Task22
    {
        ...
        {
            var regionLine = new Line(-50, 50);
            var regionCube = new Cube(regionLine, regionLine, regionLine);

            var lines = input.Split("\r\n", StringSplitOptions.RemoveEmptyEntries);
            var inputCubes = new List<CubeLight>(lines.Length);
            foreach (var line in lines)
            {
                var on = line.StartsWith("on");
                var str = line.Replace("on ", string.Empty).Replace("off ", string.Empty);
                var splits = str.Split(",", StringSplitOptions.RemoveEmptyEntries);

                var points = splits.Select(x =>
                    {
                        var trimmed = x.TrimStart('x', 'y', 'z', '=');
                        var ft = trimmed.Split("..", StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
                        return (ft[0], ft[1]);
                    })
                    .ToArray();
                ...
                var cube = new Cube
                {
                    X = new Line(points[0].Item1, points[0].Item2),
                    Y = new Line(points[1].Item1, points[1].Item2),
                    Z = new Line(points[2].Item1, points[2].Item2),
                };

                var newCube = cube.Intersect(regionCube);
                if (newCube == null) continue;

                inputCubes.Add(new CubeLight(newCube.Value, on));
            }

            var added = new List<Cube>();
            var deleted = new List<Cube>();
            foreach (var inputCube in inputCubes)
            {
                var toDelete = added.Select(x => x.Intersect(inputCube.Cube)).Where(x => x != null)
                    .Select(x => x.Value).ToArray();
                var toAdd = deleted.Select(x => x.Intersect(inputCube.Cube)).Where(x => x != null).Select(x => x.Value).ToArray();

                added.AddRange(toAdd);
                deleted.AddRange(toDelete);

                if (inputCube.On) added.Add(inputCube.Cube);
            }

            (added.Sum(x => x.Vol) - deleted.Sum(x => x.Vol)).Should().Be(expected);
        }


        struct CubeLight
        {
            public CubeLight(Cube cube, bool on)
            {
                Cube = cube;
                On = on;
            }
        ...
        [DebuggerDisplay("{Vol} - {X},{Y},{Z}")]
        struct Cube
        {
            public Cube(Line x, Line y, Line z)
            {
                X = x;
                Y = y;
                Z = z;
            ...
            public long Vol => (long)(X.To - X.From + 1) * (Y.To - Y.From + 1) * (Z.To - Z.From + 1);
            ...
            public IEnumerable<Cube> Concat(Cube other)
            {
                ...
                foreach (var x in X.Concat(other.X))
                foreach (var y in Y.Concat(other.Y))
                foreach (var z in Z.Concat(other.Z))
                {
                    var cube = new Cube(x, y, z);
                    if (cube.Intersect(this) == null &&
                        cube.Intersect(other) == null)
                        continue;
        ...
        struct Line
        {
            ...
            public Line? Intersect(Line other)
            {
                if (other.From > To || other.To < From) return null;

                return new Line(Math.Max(From, other.From), Math.Min(To, other.To));
            }

            public IEnumerable<Line> Concat(Line other)
            {
                if (Intersect(other) == null)
                {
                    yield return this;
                ...
                var points = new[] { From, To, other.From, other.To }
                    .Distinct()
                    .OrderBy(x => x)
                    .ToArray();
                ...
                foreach (var point in points)
                {
                    ...
                    yield return new Line(prevPoint.Value, point);
                    prevPoint = point;
                }
            }
        }
        ...
        {
            yield return new TestCaseData(@"on x=10..12,y=10..12,z=10..12
on x=11..13,y=11..13,z=11..13
off x=9..11,y=9..11,z=9..11
on x=10..10,y=10..10,z=10..10", 39);
            yield return new TestCaseData(@"on x=-20..26,y=-36..17,z=-47..7
...
on x=-41..9,y=-7..43,z=-33..15", 590784);
            yield return new TestCaseData(@"on x=-45..1,y=-25..23,z=-14..35
...
on x=-35094..-12809,y=66272..89209,z=22037..50934", 596598L);
        }

        [Test]
        [TestCase(1, 5, 6, 10, null, null)]
        [TestCase(6, 10, 1, 5, null, null)]
        [TestCase(1, 5, 4, 10, 4, 5)]
        [TestCase(1, 5, 2, 3, 2, 3)]
        [TestCase(2, 5, 1, 3, 2, 3)]
        [TestCase(2, 5, 1, 10, 2, 5)]
        [TestCase(2, 5, 3, 5, 3, 5)]
        [TestCase(2, 5, 2, 3, 2, 3)]
        [TestCase(2, 5, 1, 2, 2, 2)]
        [TestCase(2, 5, 5, 6, 5, 5)]
        public void TestIntersectLine(int a1, int a2, int b1, int b2, int? expected1, int? expected2)
        {
            var line1 = new Line(a1, a2);
            var line2 = new Line(b1, b2);

            var expected = line1.Intersect(line2);
            ...
            {
                expected.Should().Be(new Line(expected1.Value, expected2.Value));
            }
        }
    }
}